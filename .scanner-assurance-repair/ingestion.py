...
class EvidenceStore:
    """Tenant-scoped evidence store with replay, ordering, and policy controls.

    Valid but blocking evidence is retained as ``quarantined`` so Cloud and registry
    operators can investigate it.  It is never silently discarded and is never marked
    publishable.
    """
    ...
    def latest(
        ...
        *,
        ...
        try:
            clause = " AND publishable=1" if publishable_only else ""
            row = connection.execute(
                f"""
                SELECT envelope_json
                FROM evidence
                WHERE tenant_id=? AND subject_id=?{clause}
                ORDER BY sequence DESC
                LIMIT 1
                """,  # noqa: S608 - clause is a fixed internal literal
                (tenant_id, subject_id),
            ).fetchone()
        finally:
            connection.close()
    ...
    *,
    ...
    for item in findings:
        ...
        try:
            ...
            return Disposition.ERROR, "assurance contains an unknown severity"
    ...
    if policy.require_detonation and not detonation_observed:
        consumer = _more_restrictive(consumer, Disposition.BLOCK)
        reasons.append("a bound sandbox observation is required")
...
def _is_decision_downgrade(previous: str, current: str) -> bool:
    previous_value = _parse_disposition(previous)
    current_value = _parse_disposition(current)
