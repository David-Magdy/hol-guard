...
# These are local tool-state directories, not extension payload directories.  Package
# contents such as node_modules, dist, build, and target are intentionally inventoried:
# consumers can receive malicious code through those paths too.
...
def build_inventory(root: Path, limits: ScanLimits) -> InventoryResult:
    """Build a byte-bound inventory and digest every reachable regular file.

    Semantic analysis limits never remove a file from the artifact digest.  If the
    aggregate byte or file budget is exceeded, coverage fails closed and the digest is
    explicitly only a prefix digest because ``limit_reached`` is true.
    """
    ...
    for current, directory_names, file_names in os.walk(
        ...
        for name in sorted(directory_names):
            ...
            if stat.S_ISLNK(info.st_mode):
                ...
                target_digest = hashlib.sha256(target.encode("utf-8", errors="surrogateescape")).hexdigest()
                gaps.append(
                    CoverageGap(
                        code="INVENTORY_DIRECTORY_SYMLINK",
                        severity=Severity.MEDIUM,
                        description="Directory symlinks are bound to the artifact digest but never followed.",
                        path=relative,
                    )
                )
                _update_digest(
            ...
            if not stat.S_ISREG(info.st_mode):
                gaps.append(
                    CoverageGap(
                        ...
                        description="Special files are metadata-bound but never read by the scanner.",
                        path=relative,
                    )
                )
                entries.append(
    ...
    if limit_reached:
        gaps.append(
            CoverageGap(
                ...
                severity=Severity.HIGH,
                description=(
                    "Inventory limits were reached before the complete tree was digest-bound and inspected."
                ),
            )
        )
...
def _update_digest(
    ...
    digest.update(relative.encode("utf-8", errors="surrogateescape"))  # type: ignore[attr-defined]
    digest.update(b"\0")  # type: ignore[attr-defined]
    digest.update(str(mode & 0o177777).encode("ascii"))  # type: ignore[attr-defined]
    digest.update(b"\0")  # type: ignore[attr-defined]
    digest.update(str(size).encode("ascii"))  # type: ignore[attr-defined]
    digest.update(b"\0")  # type: ignore[attr-defined]
    digest.update(kind.encode("ascii"))  # type: ignore[attr-defined]
    digest.update(b"\0")  # type: ignore[attr-defined]
    if content_digest is not None:
        digest.update(bytes.fromhex(content_digest))  # type: ignore[attr-defined]
    digest.update(b"\0")  # type: ignore[attr-defined]
