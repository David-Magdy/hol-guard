...
def test_update_defers_when_latest_release_is_still_propagating(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda command, **_: subprocess.CompletedProcess(
            ...
            1,
            "",
            "ERROR: Could not find a version that satisfies the requirement hol-guard==2.2.3 (from versions: 1, 2.0.401)\nERROR: No matching distribution found for hol-guard==2.2.3",
        ),
    )
    ...
    assert payload["status"] == "deferred" and payload["installer_recovery"] == "trusted_python_pip"
