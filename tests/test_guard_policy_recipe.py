...
def test_policy_recipe_parses_and_safe_fixtures_pass() -> None:
    ...
    assert recipe.matcher.kind == "path"
    assert [evaluate_policy_recipe_fixture(recipe, fixture) for fixture in recipe.tests] == [
        "review",
        "unmatched",
    ]
...
def test_policy_recipe_rejects_a_fixture_that_does_not_match_expected_behavior() -> None:
    ...
    assert isinstance(tests[0], dict)
