...
from .detonation import DetonationPlan, validate_observation
from .drift import build_baseline, compare_baseline, load_baseline
from .inventory import InventoryEntry, InventoryResult, build_inventory
...
from .policy import AssurancePolicy, evaluate_decision, load_policy
...
def scan_extension_assurance(
    ...
    findings.extend(_coverage_gap_findings(inventory.gaps))
    ...
    for entry in inventory.entries:
        if not entry.readable:
            if entry.kind == "regular":
                oversized_files += 1 if entry.size > options.limits.max_file_bytes else 0
                unreadable_files += 1 if entry.size <= options.limits.max_file_bytes else 0
            else:
                opaque_files += 1
        ...
        if looks_like_archive(entry.path, prefix):
            ...
            for display_path, payload in result.text_payloads:
                nested_findings, nested_capabilities = _scan_text_payload(payload, display_path)
                findings.extend(nested_findings)
                capabilities.update(nested_capabilities)
            continue

        text_findings, text_capabilities, read_bytes = scan_text_entry(
            entry, resolved, options.limits
        )
    ...
    limitations = [
        ...
        "Native structural parsing identifies imports, sections, mitigations, and indicators; it is not full control-flow disassembly.",
        "Sandbox evidence describes only the exercised command and environment, not every reachable behavior.",
    ...
    if not detonation_observed:
        limitations.append("No bound sandbox observation was verified for this scan.")
    ...
    if opaque_files:
        gaps.append(
            CoverageGap(
                ...
                description="Files with unsupported or opaque content were inventoried but not semantically analyzed.",
                count=opaque_files,
            )
        )
        ...
        if drift_payload.get("requires_reapproval"):
            findings.append(
                SecurityFinding(
                    ...
                    description="Capabilities, commands, dependencies, native artifacts, endpoints, or security controls changed from the approved baseline.",
    ...
    layers = (
        EvidenceLayer(
            ...
            summary=f"Inventoried {len(inventory.entries)} files and {inventory.total_bytes} bytes.",
        ...
        EvidenceLayer(
            name="native-rust",
            status="complete" if native_count == rust_accelerated else "partial" if native_count else "not-applicable",
        ...
        EvidenceLayer(
            ...
            summary="Trusted DSSE signature verified." if provenance_verified else "No trusted exact-digest provenance verified.",
            metadata=provenance_payload or {},
        ),
        EvidenceLayer(
            name="sandbox",
            status="observed" if detonation_observed else "planned" if plan_present else "not-run",
            summary="A bound sandbox observation was verified." if detonation_observed else "Sandbox behavior is not established.",
            metadata=detonation_payload or {},
        ),
        EvidenceLayer(
            name="drift",
            status="changed" if drift_payload and drift_payload.get("changed") else "unchanged" if drift_payload else "no-baseline",
            summary="Compared against an approved exact-artifact baseline." if drift_payload else "No approved baseline was supplied.",
...
def _scan_text_payload(payload: bytes, display_path: str) -> tuple[list[SecurityFinding], set[str]]:
...
def _correlate(
    ...
    if surfaces.commands and any(
        finding.rule_id in {"ASSURANCE_MCP_SHELL_LAUNCHER", "ASSURANCE_MCP_MUTABLE_PACKAGE_RUNNER"}
        for finding in findings
    ):
        correlated.append(
            _correlation(
...
def _load_and_verify_provenance(
    ...
    *,
    ...
    if options.provenance_envelope_path is None:
        return None, False
    try:
        envelope = json.loads(options.provenance_envelope_path.read_text(encoding="utf-8"))
    ...
    return {"verified": result.verified, "key_id": result.key_id, "reason": result.reason}, result.verified
...
def _load_detonation(
    ...
    if options.detonation_observation_path is None:
        return {"planned": True, "observed": False, "plan_digest": plan_digest}, False, True
    try:
        observation = json.loads(options.detonation_observation_path.read_text(encoding="utf-8"))
        validate_observation(observation, expected_plan_digest=plan_digest)
...
def _dedupe_sort(findings: list[SecurityFinding]) -> list[SecurityFinding]:
    severity = {Severity.CRITICAL: 5, Severity.HIGH: 4, Severity.MEDIUM: 3, Severity.LOW: 2, Severity.INFO: 1}
