...
Direct use of `guarded-repository/action.yml` remains available for scan + SARIF + GitHub provenance, but portal registration defaults off.

## Evidence contract

Only coarse scan metadata is sent to the verifier:

- repository and exact commit SHA
- workflow run ID
- scanner version and profile
- score, grade, maximum finding severity, and finding count
- SHA-256 of the SARIF file
- generated time and evidence visibility
- fixed limitation flags
...
The registration helper requests a fresh GitHub OIDC token with audience:

`hol-guard-repository:<evidence-sha256>`

The token is sent as a bearer credential and is not embedded in the JSON evidence. The verifier must reject repository, commit, workflow-run, reusable-workflow, runner, or audience mismatches.

## Public and private modes

Public mode is intended for public GitHub repositories whose evidence subject can also be observed through GitHub's public attestations API. Public verification produces a short-lived page and badge.

Private mode stores no public URL or badge. It is appropriate when repository-associated GitHub attestation evidence is not publicly queryable.
